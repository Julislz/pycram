...
class ResponseLoader:
    def __init__(self, json_file='response.json'):
    ...
    def predict_response(self, hobby_list):
        ...
        # further processing to answer specifically
        if not answered:
            for hobby in hobby_list:

                # filter case that hobby was not detected because of grammar
                if hobby in self.data['verb_correction']:
                    hobby = self.data['verb_correction'][hobby][0]

                    # check again if hobby is known
                    if hobby in self.data['hobbies']:
                        responses = self.data['hobbies'][hobby]
                        return random.choice(responses)
                        break
